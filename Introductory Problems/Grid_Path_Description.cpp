...
int ans=0;
string s;
bool in(int x,int y){return x>=0&&x<=6&&y>=0&&y<=6;}
...
void dfs(int x,int y,int i){
    ...
    (!in(x,y+1)||vis[x][y+1])&&
    (in(x-1,y)&&!vis[x-1][y])&&
    (in(x+1,y)&&!vis[x+1][y]))return;

    if(
    (!in(x-1,y)||vis[x-1][y])&&
    (!in(x+1,y)||vis[x+1][y])&&
    (in(x,y-1)&&!vis[x][y-1])&&
    (in(x,y+1)&&!vis[x][y+1]))return;

    vis[x][y]=true;
    if(s[i]=='D'||s[i]=='?')if(in(x+1,y)&&!vis[x+1][y])dfs(x+1,y,i+1);
    if(s[i]=='U'||s[i]=='?')if(in(x-1,y)&&!vis[x-1][y])dfs(x-1,y,i+1);
    if(s[i]=='R'||s[i]=='?')if(in(x,y+1)&&!vis[x][y+1])dfs(x,y+1,i+1);
    if(s[i]=='L'||s[i]=='?')if(in(x,y-1)&&!vis[x][y-1])dfs(x,y-1,i+1);
    vis[x][y]=false;
}
void solve(){
    cin>>s;
