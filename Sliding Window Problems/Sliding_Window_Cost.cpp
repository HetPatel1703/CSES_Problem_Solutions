...
void solve(){
    ...
    auto balance = [&]() {
        ...
        if (sz(leftset)<md&&!rightset.empty()){
    ...
    auto remove_val = [&](ll val) {
        auto it=leftset.find(val);
        if (it!=leftset.end()) {
            leftsum-=val;
            leftset.erase(it);
        } else {
            it=rightset.find(val);
            rightsum -= val;
            rightset.erase(it);
        }
        balance();
    ...
    auto get_cost = [&]() -> ll {
        ll median = *leftset.rbegin();
        return median * (ll)sz(leftset)-leftsum+rightsum-median*(ll)sz(rightset);
    };
