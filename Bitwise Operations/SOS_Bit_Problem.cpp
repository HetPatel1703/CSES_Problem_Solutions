...
void solve(){
    ...
    // SOS DP for Submasks (x | y = x)
    frf(bit,0,20){
        frf(mask,0,1<<20){
            if(mask&(1LL<<bit)){
                dpsub[mask]+=dpsub[mask^(1<<bit)];
            }
        }
    }
    // SOS DP for Supermasks (x & y = x)
    frf(bit,0,20){
        frf(mask,0,1<<20){
            if(!(mask&(1LL<<bit))){
                dpsup[mask]+=dpsup[mask|(1<<bit)];
            }
        }
    }
    frf(i,0,n){
