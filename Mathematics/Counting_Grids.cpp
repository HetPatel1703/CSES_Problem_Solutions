...
void solve(){
    ...
    if(n & 1){
        ll half=(n / 2) % P;
        ll halfplus = (n / 2 + 1) % P;
        c90 = (half * halfplus % P + 1) % P;
        c180 = ((2 * (c90 - 1) % P + 1) % P + P) % P;
    } else {
        ll half = (n / 2) % P;
        c90 = (half * half) % P;
        c180 = (2 * c90) % P;
    }
    int ans = mod_pow(2, c0);
    ans = mod_add(ans, mod_mul(2, mod_pow(2, c90)));
    ans = mod_add(ans, mod_pow(2, c180));
    ans = mod_div(ans, 4);
    oe(ans);
}
