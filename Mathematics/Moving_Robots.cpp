...
vvll Matrixmultiply(vvll &A,vvll &B){
    int n = A.size();
    int m = B[0].size();
    int k = B.size();
...
vvll MatrixPow(vvll A, long long p) {
    int n = A.size();
...
void solve(){
    i1(k);
    vector<vector<lld>>ans(8,vector<lld>(8,1.0));
    frf(r,0,8){
        frf(c,0,8){
            vector<vector<lld>>dp(8,vector<lld>(8,0.0));
            //If Robot started at (r,c)
            //dp[i][j] be probability that this robot is at (i,j) after k moves
            dp[r][c]=1.0;
            frf(step,0,k){
                vector<vector<lld>>ndp(8,vector<lld>(8,0.0));
                frf(i,0,8){
                    frf(j,0,8){
                        if(!dp[i][j])continue;
                        int c=0;
                        frf(d,0,4){
                            int ni=i+dx4[d];
                            int nj=j+dy4[d];
                            if(ni>=0&&ni<8&&nj>=0&&nj<8)c++;
                        }
                        frf(d,0,4){
                            int ni=i+dx4[d];
                            int nj=j+dy4[d];
                            if(ni>=0&&ni<8&&nj>=0&&nj<8){
                                ndp[ni][nj]+=(lld)dp[i][j]/c;
                            }
                        }
                    }
                }
