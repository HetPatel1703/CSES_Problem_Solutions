...
vector<string>b59(5,string(9,'.'));
vector<string>b95(9,string(5,'.'));
...
bool solve59(int r,int c,char cur){
    if(r==5)return true;
    if(c==9)return solve59(r+1,0,cur);
    if(b59[r][c]!='.')return solve59(r,c+1,cur);
    ...
    for(int i=0;i<4;++i){
        ...
        if(r1>=0&&r1<5&&c1>=0&&c1<9&&b59[r1][c1]=='.'&&
           r2>=0&&r2<5&&c2>=0&&c2<9&&b59[r2][c2]=='.'){
            
            b59[r][c]=b59[r1][c1]=b59[r2][c2]=cur;
            if(solve59(r,c+1,cur+1))return true;
            b59[r][c]=b59[r1][c1]=b59[r2][c2]='.';
        }
    }
    return false;
}

void init_base_blocks(){
...
void solve(){
    ...
    vector<string>b23={"AAB","ABB"};
    vector<string>b32={"AA","BA","BB"};

    auto place = [&](int r,int c,const vector<string>&b){
        ...
        for(auto& [ochar,coords]:mp){
            ...
            for(auto& p:coords){
                ans[p.ff][p.ss]=chosen;
            }
        }
    };

    auto fill=[&](auto&& fill,int r1,int r2,int c1,int c2){
        int h=r2-r1;
        int w=c2-c1;
        if(!h||!w)return;
        ...
        //if odd*odd then use of 5*9 or 9*5

        if(h%3==0){
            place(r1,c1,b95);
            fill(fill,r1,r1+9,c1+5,c2);
            fill(fill,r1+9,r2,c1,c2);
            return;
        }
        if(w%3==0){
            place(r1,c1,b59);
            fill(fill,r1,r1+5,c1+9,c2);
            fill(fill,r1+5,r2,c1,c2);
            return;
        }
    };
