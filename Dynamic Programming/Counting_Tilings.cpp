...
vvll dp1;
void dfs(int r,int mask,int nmask,vll&ndp,int n){
    if(r==n){
        ndp.pb(nmask);
        return;
    }
    if(mask&(1<<r)){
        dfs(r+1,mask,nmask,ndp,n);
        return;
    }
    if(r+1<n&&!(mask&(1<<(r+1)))){
        dfs(r+2,mask,nmask,ndp,n);
    }
    dfs(r+1,mask,nmask|(1<<r),ndp,n);
}
void solve(){
    i2(n,m);
    vvll dp(m+1,vll(1<<n));
    ...
    frf(c,0,m){
        frf(mask,0,1<<n){
            if(!dp[c][mask])continue;
            vll ndp;
            dfs(0,mask,0,ndp,n);
            for(auto it:ndp){
                dp[c+1][it]=mod_add(dp[c+1][it],dp[c][mask]);
            }
        }
    }oe(dp[m][0]);
}
