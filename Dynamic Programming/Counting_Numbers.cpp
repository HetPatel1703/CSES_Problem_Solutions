...
vll d;
int dp[20][11][2][2];
int dfs(int pos,int prev,int tight,int leadz){
    if(pos==d.size()) return 1;

    if(dp[pos][prev][tight][leadz]!=-1)
        return dp[pos][prev][tight][leadz];
    ...
    for(int dig=0;dig<=limit;dig++){
        ...
        if(!leadz && dig==prev) continue;

        int nextLead = leadz && (dig==0);
        int nextPrev = nextLead ? 10 : dig;

        ans += dfs(pos+1,nextPrev,tight && (dig==limit),nextLead);
    }

    return dp[pos][prev][tight][leadz]=ans;
}
int fun(long long n){
